...
import { StyleSheet, ScrollView, Dimensions, TouchableOpacity, ToastAndroid, View } from "react-native";
...
let userid
let result = []
export default withNavigation(class Home extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      News: data,
    ...
    return this.state.News.map(element => {
      return (
        <Card style={styles.card} onPress={() => this.setState({ showURL: true })}>
          ...
          />
          <TouchableOpacity onPress={() => { this.openWebView(element.link);
            console.log('clicked'); }}>
